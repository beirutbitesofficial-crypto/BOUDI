...
import {api} from '../api.js?v=20260912-mobile';import {el,statCard,money,num,fmtDate,badge,modal} from '../ui.js?v=20260912-mobile';import {renderSupplierInvoices} from './supplier-invoices.js?v=20260912-mobile';
export async function renderInventory(root){
  ...
  async function overview(){const [inv,alerts,history]=await Promise.all([api.get('/inventory'),api.get('/inventory/alerts'),api.get('/inventory/history')]);body.appendChild(el('div.grid.stats-grid',{},[
    ...
    body.appendChild(el('div.card',{},[el('h3',{text:t('Stock History')}),...history.slice(0,100).map(h=>el('div.history-line',{},[el('b',{text:h.product_name}),el('span',{text:(h.change>=0?'+':'')+h.change}),el('span',{text:t(h.reason)}),el('span',{text:t(h.ref||'-')}),el('small',{text:fmtDate(h.created_at)})]))]));
  }await show('overview');
}
